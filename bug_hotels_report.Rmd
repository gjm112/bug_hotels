...
library(ggmosaic)
library(tidyverse)
cong <- read.csv("data/cong_master.csv")
cong %>% group_by(Week, Date) %>% summarize(n = n())
...
datbeta <- data.frame(treatment = rep(c("Meadow","Typhd","Typha"), each = 4*3000), 
                   category = rep(c("Collector", "Shredder", "Piercer", "Scraper"),each = 3000),
...
ggplot(aes(x = beta), data = datbeta) + geom_density() + facet_grid(treatment ~ category) + geom_vline(xintercept = 0) + xlab(expression(beta)) + ylab("") # + ggtitle("All relative to Submerged")
ggsave("FigureY.pdf",height=5,width=8)
```
...
```{r}
summ <- datbeta %>% group_by(treatment, category) %>% summarize(OR_post_mean = quantile(exp(beta),0.5),
...
for (i in 1:1000){
  results1[[i]] <-
    data.frame(Category = rep(c("Collector","Shredder","Piercer","Scraper","Predator"),4),                treatment = rep(c("Submerged","Meadow","Typhd","Typha"),each = 5),
               p = c(apply(cbind(exp(as.matrix(newX) %*% z$beta[,,i,1]),1),1,function(x) {x / sum(x)})))

results2[[i]] <- data.frame(Category = rep(c("Collector","Shredder","Piercer","Scraper","Predator"),4),                treatment = rep(c("Submerged","Meadow","Typhd","Typha"),each = 5),
               p = c(apply(cbind(exp(as.matrix(newX) %*% z$beta[,,i,1]),2),1,function(x) {x / sum(x)})))

results3[[i]] <- data.frame(Category = rep(c("Collector","Shredder","Piercer","Scraper","Predator"),4),                treatment = rep(c("Submerged","Meadow","Typhd","Typha"),each = 5),
...
ggplot(aes(x = p, y = after_stat(scaled), color = Category), data = results) + geom_density() + facet_wrap(~treatment )  + theme_bw() + ylab("") # +ggtitle("oxygen = 1")
ggsave("FigureZ.pdf",height=5,width=8)

summ <- results %>% 
  group_by(treatment,Category) %>% 
  summarize(OR_post_mean = mean(p),
            OR_post_mean_LB = quantile(p,0.025),
  OR_post_mean_UB = quantile(p,0.975)
)
summ
```
