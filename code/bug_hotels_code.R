...
results1[[i]] <- data.frame(category = rep(c("p_col", "p_shr", "p_pie", "p_scr", "p_pre"),4), treatment = rep(c("meadow","submerged","typhd","typha"),each = 5), p = c(apply(exp(as.matrix(newX) %*% z$beta[,,i,1]),1,function(x){x/sum(x)})))
results2[[i]] <- data.frame(category = rep(c("p_col", "p_shr", "p_pie", "p_scr", "p_pre"),4), treatment = rep(c("meadow","submerged","typhd","typha"),each = 5), p = c(apply(exp(as.matrix(newX) %*% z$beta[,,i,2]),1,function(x){x/sum(x)})))
results3[[i]] <- data.frame(category = rep(c("p_col", "p_shr", "p_pie", "p_scr", "p_pre"),4), treatment = rep(c("meadow","submerged","typhd","typha"),each = 5), p = c(apply(exp(as.matrix(newX) %*% z$beta[,,i,3]),1,function(x){x/sum(x)})))
}

z$beta[5,1:5,1:1000,1][,1:3]

results <- rbind(do.call(rbind,results1),do.call(rbind,results2),do.call(rbind,results3))
#ggplot(aes(x = p, fill = category), data = results) + geom_histogram() + facet_grid(treatment ~ 1)
ggplot(aes(x = p, y = after_stat(scaled), color = category), data = results) + geom_density() + facet_grid(treatment ~ 1)

#ggplot(aes(x = p, color = category), data = results) + geom_boxplot() + facet_grid(treatment ~ 1)
#ggplot(aes(x = p), data = results) + geom_density() + facet_grid(treatment ~ category)
